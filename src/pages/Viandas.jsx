...
import { C } from '../theme'
import { IconMeat, IconBox } from '../components/icons'
  ...
  return (
    <div style={{ color: C.textPrimary }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        <NuevaButton onClick={abrirNueva}>+ Nueva</NuevaButton>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '14px', padding: '2.5rem', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '10px' }}><IconBox size={34} color={C.textMuted} /></div>
          <div style={{ color: C.textSecondary, fontSize: '14px' }}>No tenés viandas cargadas todavía.</div>
          <div style={{ marginTop: '1rem' }}><NuevaButton onClick={abrirNueva}>Agregar primera vianda</NuevaButton></div>
        </div>
      ) : (
            ...
            return (
              <div key={v.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                      <IconMeat size={16} color={C.textMuted} />
                  ...
                  <div style={{ textAlign: 'center', minWidth: '52px', marginLeft: '12px' }}>
                    <div style={{ fontSize: '26px', fontWeight: 800, lineHeight: 1, color: agotada ? C.red : C.accentText, fontVariantNumeric: 'tabular-nums' }}>{v.portions}</div>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <StepButton onClick={() => ajustarPorciones(v.id, -1)} disabled={v.portions === 0}>−</StepButton>
                  <StepButton onClick={() => ajustarPorciones(v.id, 1)} accent>+</StepButton>
                  <div style={{ flex: 1 }} />
                  <SmallGhostButton onClick={() => abrirEditar(v)}>Editar</SmallGhostButton>
                  <SmallDangerButton onClick={() => eliminar(v.id)}>✕</SmallDangerButton>
                </div>
              </div>
            )
          })}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: C.surface, borderRadius: '20px 20px 0 0', width: '100%', maxWidth: '480px', maxHeight: '85vh', overflowY: 'auto', padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
              <ModalPrimaryButton onClick={guardar} disabled={saving || !form.name}>{saving ? 'Guardando…' : 'Guardar'}</ModalPrimaryButton>
...
function NuevaButton({ onClick, children }) {
  const { style, handlers } = useInteractiveStyle(
    { background: C.accent, color: 'white', border: 'none', padding: '9px 16px', borderRadius: '10px', cursor: 'pointer', fontWeight: 700, fontSize: '14px' },
    { hover: { filter: 'brightness(1.08)' }, focus: focusRing }
  )
  return <button onClick={onClick} style={style} {...handlers}>{children}</button>
}

function StepButton({ onClick, disabled, accent, children }) {
  const { style, handlers } = useInteractiveStyle(
    {
      width: '34px', height: '34px', border: `1px solid ${accent ? C.accent + '40' : C.border}`, borderRadius: '8px',
      cursor: 'pointer', fontWeight: 700, fontSize: '16px',
      background: accent ? C.accentDim : C.surfaceHigh, color: accent ? C.accentText : C.textSecondary,
      display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: disabled ? 0.4 : 1,
    },
    { hover: disabled ? null : { background: accent ? C.accent : C.border, color: accent ? '#fff' : C.textPrimary }, focus: focusRing }
  )
  return <button onClick={onClick} disabled={disabled} style={style} {...handlers}>{children}</button>
}

function SmallGhostButton({ onClick, children }) {
  const { style, handlers } = useInteractiveStyle(
    { fontSize: '12px', border: `1px solid ${C.border}`, padding: '5px 12px', borderRadius: '8px', cursor: 'pointer', background: 'transparent', color: C.textSecondary },
    { hover: { borderColor: C.textMuted, color: C.textPrimary }, focus: focusRing }
  )
  return <button onClick={onClick} style={style} {...handlers}>{children}</button>
}

function SmallDangerButton({ onClick, children }) {
  const { style, handlers } = useInteractiveStyle(
    { fontSize: '12px', border: `1px solid ${C.red}40`, color: C.red, padding: '5px 12px', borderRadius: '8px', cursor: 'pointer', background: 'transparent' },
